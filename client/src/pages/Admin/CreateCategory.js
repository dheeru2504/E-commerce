...
const CreateCategory = () => {
  ...
  const handleSubmit = async (ev) => {
    ev.preventDefault();
    try {
      const { data } = await axios.post("https://backend-k2am.onrender.com/api/v1/category/create-category", {
        name,
      });
      if (data?.success) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `https://backend-k2am.onrender.com/api/v1/category/update-category/${selected._id}`,
        { name: updatedName }
      );
      if (data?.success) {
  ...
  const handleDelete = async (pId) => {
    try {
      const { data } = await axios.delete(
        `https://backend-k2am.onrender.com/api/v1/category/delete-category/${pId}`
      );
      if (data.success) {
